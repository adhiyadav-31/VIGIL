...
from backend.ml.forecasting import forecast_cashflow
from backend.services.analytics_service import compute_business_health_score
...
def _generate_recommendation(
    ...
    """Generate dynamic actionable advice based on simulated parameters."""
    breach = cashflow.get("runway_breach_week")
    if breach is not None:
        return f"CRITICAL: Cash runway is breached in Month {breach + 1}. Reduce machinery capex or avoid taking large loans until operating margin improves."
    
    if risk_band == "High":
        return f"HIGH RISK (Score: {risk_score}/100): Fixed monthly expenses of ₹{payload.monthly_expenses:,.0f} and lever costs leave minimal cash buffer. Consider phasing hires or equipment acquisition over two quarters."
    
    if monthly_profit < 0:
        return f"MODERATE RISK: Net operating cash burn is -₹{abs(monthly_profit):,.0f}/month. Ensure sufficient cash reserve before committing to fixed expenses."
    
    if risk_band == "Moderate":
        return f"VIABLE WITH CAUTION (Score: {risk_score}/100): Projected net monthly profit is +₹{monthly_profit:,.0f}. Maintain a 30-day liquid reserve."

    return f"FINANCIALLY SOUND (Score: {risk_score}/100): Strong revenue coverage (+₹{monthly_profit:,.0f}/month profit). Projected 6-month ending cash balance reaches ₹{cashflow['ending_balance']:,.0f}."
...
@router.get("/run")
@router.get("")
async def run_simulation(payload: SimulatorRequest = None):
    if payload is None:
        payload = SimulatorRequest()
    try:
        ...
        payroll_impact = payload.hire * AVG_MONTHLY_SALARY
        machinery_maint = payload.machinery * MACHINERY_MONTHLY_MAINTENANCE_RATE
        loan_emi = payload.loan * LOAN_MONTHLY_EMI_RATE
        product_cost = NEW_PRODUCT_MONTHLY_COST if payload.new_product else 0
        branch_cost = NEW_BRANCH_MONTHLY_COST if payload.new_branch else 0

        adjusted_expenses += payroll_impact + machinery_maint + loan_emi + product_cost + branch_cost
        ...
        recommendation_text = _generate_recommendation(
            payload, monthly_profit, cashflow, risk_score, risk_band
        )

        months_labels = [f"Month {i + 1}" for i in range(PROJECTION_MONTHS)]
        ...
        return {
            ...
            "risk": risk_band,
            "risk_details": {"score": risk_score, "band": risk_band, "source": risk_source},
            "business_health_score": health_score,
            "probability_of_recovery": probability_of_recovery,
            "recommendation": recommendation_text,
            "cashflow": cashflow,
            "ending_cash": round(cashflow["ending_balance"], 2),
            "cost_breakdown": {
                "payroll": payroll_impact,
                "machinery_maintenance": machinery_maint,
                "loan_emi": loan_emi,
...
def _estimate_risk(monthly_revenue: float, monthly_expenses: float, cash_balance: float):
    ...
    from backend.ml.risk_prediction import predict_risk_score
