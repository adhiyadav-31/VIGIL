...
Called by:
    - backend/main.py (startup env validation, app factory)
...
# Project root = two levels up from this file (backend/config.py -> app/ -> root)
...
class Settings:
    ...
    def validate_required(self) -> None:
        """Fail-fast check used by backend/main.py at startup. Logs a warning
        if a required setting is missing so the UI can still be demoed."""
        missing = []
        if not self.gemini_api_key:
            missing.append("GEMINI_API_KEY")
        if missing:
            import logging
            logger = logging.getLogger("smart_automation_ai.config")
            logger.warning(
                "Missing environment variable(s): %s. "
                "AI agents will use fallback logic. Set them in %s.",
                ", ".join(missing), ENV_PATH,
            )
            print(
                f"WARNING: Missing env var(s): {', '.join(missing)}. "
                f"Set them in {ENV_PATH}. AI features will use fallbacks."
            )
