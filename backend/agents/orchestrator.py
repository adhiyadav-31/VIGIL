...
Called by: app/api/agents.py (`from backend.agents.orchestrator import build_graph, AgentState`)
"""
...
async def _finance_node(state: AgentState) -> AgentState:
    from backend.agents.finance_agent import analyze
...
async def _scheme_node(state: AgentState) -> AgentState:
    from backend.agents.scheme_agent import recommend_schemes
...
async def _recovery_node(state: AgentState) -> AgentState:
    from backend.agents.recovery_agent import analyze
...
async def _growth_node(state: AgentState) -> AgentState:
    from backend.agents.growth_agent import analyze
    outputs = state["specialist_outputs"]
...
async def _operations_node(state: AgentState) -> AgentState:
    state["specialist_outputs"]["operations"] = {
        "agent": "Operations Agent",
        "confidence": 85,
        "recommendations": [{
            "action": "Diversify supplier base for top 3 SKUs",
            "problem": "Single-supplier dependency flagged in recent operational signals",
            "cause": "Supplier delays reported in recent check-ins"
        }],
        "risk_band": "Low",
        "summary": "Diversify supplier base for top 3 SKUs"
    }
    return state


async def _ceo_node(state: AgentState) -> AgentState:
    from backend.agents.ceo_agent import synthesize
...
def build_graph():
    ...
    finance -> risk -> competitor -> schemes -> recovery -> growth -> operations -> ceo
    """
    ...
    graph.add_node("operations", _operations_node)
    ...
    graph.add_edge("growth", "operations")
    graph.add_edge("operations", "ceo")
